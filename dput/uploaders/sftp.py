    ...
    def initialize(self, **kwargs):
        ...
        try:
            ...
            if self._config[Opt.KEY_SFTP_PRIVATE_KEY]:
                ...
                logger.debug("Authenticate using private key %s" % (private_key_file))

                if not os.access(private_key_file, os.R_OK):
                    raise SftpUploadException("Key file %s is not accessible" % (private_key_file))
                private_key = paramiko.RSAKey.from_private_key_file(private_key_file)

            user = self._config[Opt.KEY_SFTP_USERNAME]
            password = None

            logger.debug("SFTP user: %s; password: %s" % (user, "YES" if password else "NO"))

            self._transport.connect(username=user, password=password, pkey=private_key)
            self._sftp = paramiko.SFTPClient.from_transport(self._transport)
        except paramiko.AuthenticationException as e:
            raise SftpUploadException("Failed to authenticate with server %s: %s" % (self._config[Opt.KEY_FQDN], e))


        try:
            self._sftp.chdir(self._config[Opt.KEY_INCOMING])
        except IOError as e:
            raise SftpUploadException("Could not change directory to %s: %s" % (self._config[Opt.KEY_INCOMING], e))
    ...
    def upload_file(self, filename):
        ...
        try:
            ...
            if e.errno == 13:
                ...
                raise SftpUploadException("Could not upload file %s: %s" % (filename, e))
